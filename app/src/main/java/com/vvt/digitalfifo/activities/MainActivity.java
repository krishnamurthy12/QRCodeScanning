...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, OnResponseListener {
    ...
    private void initializeViews() {
        ...
        mLineName=findViewById(R.id.vT_am_line_name);
        mStationName=findViewById(R.id.vT_am_station_name);

        mScannedText=findViewById(R.id.vT_am_result_textview);

        mLogOut=findViewById(R.id.vL_am_logout);
        mLogOut.setOnClickListener(this);

        mProgressBar=findViewById(R.id.vP_am_progressbar);
        mProgressBar.setVisibility(View.GONE);

        mSuccessLayout=findViewById(R.id.vL_am_success_layout);
        mFailureLayout=findViewById(R.id.vL_am_failure_layout);

        mSuccessMessage=findViewById(R.id.vT_am_success_msg);
        mFailureMessage=findViewById(R.id.vT_am_failure_msg);

        mScanLayout=findViewById(R.id.vL_al_scan_layout);
      ...
      /*  mSuccess=findViewById(R.id.btn_success);
        ...
        String [] logInDetails=Utilities.getIsLoggedIn(MainActivity.this);
                        ...
                        if(scannedtext.startsWith("@") && scannedtext.endsWith("@"))
                        {
                            ...
                            try {
                                if(scannedDetails.length>=2)
                                {
                                   ...
                                   /* if(stationName.equalsIgnoreCase("analysis") || stationName.equalsIgnoreCase("loms"))
                                    {
                                        callValidateKanbonAPIForAnalysisAndLoms(lineName,stationName,partNumber,quantity,kanbanNumber);
                                    }
                                    else if(stationName.equalsIgnoreCase("quarantine") )
                                    {
                                        callValidateKanbonAPIForQuarantine(lineName,stationName,partNumber,quantity,kanbanNumber);
                                    }
                                    else {
                                        callValidateKanbonAPI(lineName,stationName,partNumber,quantity,kanbanNumber);
                                    }*/
    ...
    private void callValidateKanbonAPI(String lineName,String stationName,String partNumber,String quantity,String kanbonNumber) {
        ...
        if (Utilities.isConnectedToInternet(getApplicationContext())) {
            ...
            WebServices<KanbonScanResponse> webServices = new WebServices<KanbonScanResponse>(MainActivity.this);
            webServices.validateKanbon(Utilities.getBaseURL(MainActivity.this), WebServices.ApiType.kanbonScan, kanbonScan);

        } else {
            Utilities.showToast(MainActivity.this,getResources().getString(R.string.err_msg_nointernet));
        }
    }

   /* private void callValidateKanbonAPIForAnalysisAndLoms(String lineName,String stationName,String partNumber,String quantity,String kanbonNumber) {
        // StationName stationName = new StationName(lineName);
        ...
        if (Utilities.isConnectedToInternet(getApplicationContext())) {
            ...
            WebServices<KanbonScanResponse> webServices = new WebServices<KanbonScanResponse>(MainActivity.this);
            webServices.analysisAndLomsOut(Utilities.getBaseURL(MainActivity.this), WebServices.ApiType.kanbonScanForAnalysisAndLoms, kanbonScan);
    ...
    private void callValidateKanbonAPIForQuarantine(String lineName,String stationName,String partNumber,String quantity,String kanbonNumber) {
        // StationName stationName = new StationName(lineName);
        ...
        if (Utilities.isConnectedToInternet(getApplicationContext())) {
            ...
            WebServices<KanbonScanResponse> webServices = new WebServices<KanbonScanResponse>(MainActivity.this);
            webServices.quarantineOut(Utilities.getBaseURL(MainActivity.this), WebServices.ApiType.kanbonScanForAnalysisAndLoms, kanbonScan);
    ...
    private void callLogOutAPI(String lineName, String stationName) {
       // StationName stationName = new StationName(lineName);
        ...
        if (Utilities.isConnectedToInternet(getApplicationContext())) {
            ...
            WebServices<LogOutResponse> webServices = new WebServices<LogOutResponse>(MainActivity.this);
            webServices.logOut(Utilities.getBaseURL(MainActivity.this), WebServices.ApiType.logOut, logOut);
                ...
                if(isSucces)
                {
                    ...
                    if(kanbonScanResponse!=null)
                    {
                        if(kanbonScanResponse.getMessage()!=null && kanbonScanResponse.getStatus()!=null)
                        {
                            ...
                            showSnackBar(MainActivity.this,"Something went wrong please try again");
                        }

                    }
                    else {
                        showSnackBar(MainActivity.this,"Server is busy");
                    }

                }
                else {
                    //API call failed
                    showSnackBar(MainActivity.this,"Server Timeout");
                }
                break;

           /* case kanbonScanForAnalysisAndLoms:
                toggleVisibility(false,mProgressBar);
                if(isSucces)
                {
                    KanbonScanResponse kanbonScanResponse= (KanbonScanResponse) response;
                ...
                if(isSucces)
                {
                    ...
                    if(logOutResponse!=null)
                    {
                        if(logOutResponse.getStatus()!=null && logOutResponse.getMessage()!=null)
                        {
                            ...
                            {
                                //Success
                                saveLogInPreference(MainActivity.this,false);

                                startActivity(new Intent(MainActivity.this,LoginActivity.class));
                                this.finish();
                            }
                            else {
                                showSnackBar(MainActivity.this,logOutResponse.getMessage());
                            }
                        }
                        else {
                            showSnackBar(MainActivity.this,"Something went wrong please try again");
